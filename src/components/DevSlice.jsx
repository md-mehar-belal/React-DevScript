...
import { removeFromDevSlices } from "../redux/devSlice";
import { FormatDate } from "../utlis/formatData";

const devSlice = () => {
  const devSlices = useSelector((state) => state.devSlice.devSlices);

  const [searchTerm, setSearchTerm] = useState("");

  const dispatch = useDispatch();

  const handleDelete = (devSliceId) => {
    dispatch(removeFromDevSlices(devSliceId));
  };

  // Filter pastes based on search term (by title or content)
  const filtereData = devSlices.filter(
    (devSlice) =>
      devSlice.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
      devSlice.content.toLowerCase().includes(searchTerm.toLowerCase())
  );
              ...
              filtereData.map((devSlice) => (
                ...
                >
                  ...
                  <div className="w-[50%] flex flex-col space-y-3">
                    <p className="text-4xl font-semibold ">{devSlice?.title}</p>
                    <p className="text-lg font-normal line-clamp-3 max-w-[80%] text-[#707070]">
                      {devSlice?.content}
                    </p>
                  </div>
                  ...
                  <div className="flex flex-col gap-y-4 sm:items-end">
                    <div className="flex gap-2 flex-wrap sm:flex-nowrap">
                      ...
                      >
                        <a href={`/?devSliceId=${devSlice?._id}`}>
                          <PencilLine
                      ...
                      <button
                        ...
                        onClick={() => handleDelete(devSlice?._id)}
                      >
                        <Trash2
                      ...
                      <button className="p-2 rounded-[0.2rem] bg-white border border-[#c7c7c7]  hover:bg-transparent group hover:border-orange-500">
                        <a href={`/devSlices/${devSlice?._id}`} target="_blank">
                          <Eye
                      ...
                      <button
                        ...
                        onClick={() => {
                          navigator.clipboard.writeText(devSlice?.content);
